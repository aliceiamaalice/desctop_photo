...
window = Tk()
window.title("это моя программа она принадлежит мне")
...
def selected(event): # event-обытие
...
path_photo = f"{os.getcwd()}C:\\Photo\\"
...
combobox = ttk.Combobox(values=names, textvariable=photo_var)#textvariable-предяет текущий момент Combobox
combobox.bind("<<ComboboxSelected>>", selected) # bind-привязка
